...
def build_morning_msg(results):
    ...
    combo = [s for s in results if s["fundamental_lolos"] and s["bandar_akum"]][:2]
    ...
    if combo:
        lines.append("💎 <b>FUNDAMENTAL KUAT + BANDAR AKUM (terbaik):</b>")
        for s in combo:
            lines.append(f"   ⭐ <b>{s['ticker']}</b> Rp{s['price']:,.0f} | F.Skor: {s['fundamental_skor']} | Akum: {s['bandar_accum_days']}h\n")
    if kuat:
        lines.append("🔥 <b>SINYAL BANDAR KUAT:</b>")
        for s in kuat:
            lines.append(f"   • <b>{s['ticker']}</b> Rp{s['price']:,.0f} | Akum {s['bandar_accum_days']} hari\n")
    if scalp:
        lines.append("⚡ <b>KANDIDAT SCALPING:</b>")
        for s in scalp:
            lines.append(f"   • <b>{s['ticker']}</b> Rp{s['price']:,.0f} | Range {s['avg_range']:.1f}% | Vol {s['vol_ratio']:.1f}x\n")
    lines.append("Ketik /combo /pagi /akum /fa untuk detail")
...
def build_help_msg():
    return (
        ...
        "💎 /combo — Fundamental kuat + Bandar akumulasi (terbaik)\n"
        "📊 /fa    — Saham fundamental kuat (PER/ROE/DER bagus)\n"
        "📈 /akum  — Akumulasi bandar AK/MG (swing)\n"
        "⚡ /pagi  — Beli pagi jual sore (scalping)\n"
        "🔄 /scan  — Paksa scan sekarang\n"
        "📋 /status — Status bot\n"
        "❓ /help  — Perintah ini\n\n"
        "💡 <b>Tips:</b> Gunakan /combo untuk saham terbaik\n"
        "(Fundamental bagus + Bandar sedang akumulasi = entry ideal)"
    )

# ── SCAN ─────────────────────────────────────────────────
def run_scan(fetch_fundamental=False):
    global prev_signals, last_results
    total = len(STOCKS)
    results = []
    ok = 0
    print(f"[{now_wib().strftime('%H:%M')} WIB] Scanning {total} saham (fundamental={'ya' if fetch_fundamental else 'cache'})...")

    for i, ticker in enumerate(STOCKS):
        try:
            # Ambil fundamental (sekali sehari atau kalau diminta)
            if fetch_fundamental or ticker not in fundamental_cache:
    ...
    combos   = [r for r in results if r["fundamental_lolos"] and r["bandar_akum"]]
    ...
    if new_sigs:
        ...
        if combos:
            send(f"💎 <b>BONUS — {len(combos)} saham di atas juga LOLOS filter fundamental!</b>\nKetik /combo untuk detail.")
    elif not signals:
...
def webhook():
    try:
        ...
        print(f"CMD: {text} dari {cid}")

        if text == "/pagi":
            send(build_pagi_msg(last_results) if last_results else "⏳ Tunggu scan selesai.", cid)
        elif text == "/akum":
            send(build_akum_msg(last_results) if last_results else "⏳ Tunggu scan selesai.", cid)
        elif text == "/combo":
            send(build_fundamental_msg(last_results) if last_results else "⏳ Tunggu scan selesai.", cid)
        elif text == "/fa":
            send(build_fa_msg(last_results) if last_results else "⏳ Tunggu scan selesai.", cid)
        elif text == "/scan":
            send("🔄 Scanning... tunggu 3-5 menit.", cid)
            Thread(target=lambda: (run_scan(True), send("✅ Scan selesai! Ketik /combo /pagi /akum /fa", cid))).start()
        elif text == "/status":
            combos = len([r for r in last_results if r["fundamental_lolos"] and r["bandar_akum"]])
            send(
                f"✅ <b>Bot aktif</b>\n"
                f"⏰ {now_wib().strftime('%d/%m/%Y %H:%M')} WIB\n"
                f"📊 Dipantau: {len(STOCKS)} saham\n"
                f"📋 Data: {len(last_results)} saham\n"
                f"🏦 Sinyal bandar: {len([r for r in last_results if r['bandar_akum']])}\n"
                f"💎 Combo (F+T): {combos}", cid
            )
...
def home():
    ...
    return (
        ...
        f"<p style='color:#94a3b8'>Combo (Fundamental+Bandar): {combos}</p>"
        f"<p style='color:#94a3b8'>Waktu: {now_wib().strftime('%d/%m/%Y %H:%M:%S')} WIB</p>"
        f"<p style='color:#94a3b8'>CMD: /combo /pagi /akum /fa /scan /status /help</p>"
        f"</body></html>"
    )
...
def main_loop():
    ...
    send(
        ...
        f"💎 Fundamental (PER/ROE/DER) + Broker Flow\n\n"
        f"<b>Perintah:</b>\n"
        f"💎 /combo — Fundamental kuat + Bandar akum\n"
        f"📊 /fa    — Fundamental kuat saja\n"
        f"📈 /akum  — Akumulasi bandar\n"
        f"⚡ /pagi  — Scalping harian\n"
    ...
    while True:
        ...
        if is_wdays and n.hour == 8 and 44 <= n.minute <= 46 and morning_sent != today:
            results = run_scan(fetch_fundamental=True)  # refresh fundamental pagi
